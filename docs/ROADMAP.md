...
- [ ] 서버 컴포넌트에서 렌더링 (React DevTools에서 서버 컴포넌트 확인)
- [ ] 데스크탑 3열 / 태블릿 2열 / 모바일 1열 반응형 그리드 확인

---

## Phase 4: 추가 기능 (상태: 부분 완료) — 2~3일
...
- [ ] 카테고리 탭 클릭 시 해당 카테고리 기술만 표시됨
- [ ] 태그 토글 클릭 시 해당 태그를 가진 기술만 표시됨
- [ ] 필터 초기화 버튼 클릭 시 전체 목록으로 복귀됨
- [ ] 홈 검색창에 키워드 입력 후 엔터 시 `/search?q=[keyword]`로 이동
- [ ] 검색 결과 페이지에서 매칭 기술 카드 목록 표시
- [ ] 검색 결과 없을 시 Empty State 메시지 표시
- [ ] 빠른 타이핑 시 300ms 디바운스로 API 호출 횟수 제한 확인

---

## Phase 5: 최적화 및 배포 (상태: 미시작) — 2일
...
- [ ] 홈 페이지 FCP 2초 이내 (Chrome Lighthouse 기준)
- [ ] Lighthouse 성능 점수 90점 이상
- [ ] Notion API 응답 대기 중 스켈레톤 UI 표시 확인
- [ ] Vercel 배포 URL에서 전체 페이지 정상 동작
- [ ] `.env.local` 미포함 확인 (`git log` 검사)
